...
stages {
 ...
 steps {
     TASK_DEFINATION = sh (aws ecs describe-task-definition --task-definition "hello_world" --region "us-east-1")
     NEW_TASK_DEFINTIION =sh (echo $TASK_DEFINITION | jq --arg IMAGE "$NEW_IMAGE" '.taskDefinition | .containerDefinitions[0].image = $IMAGE | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)')
